 ...
 *
 * lineHeight is absolute px (not a ratio).
...
/**
 * Base font family, per platform.
 *
 * The two platforms resolve font names differently, and Wanted Sans does not embed the
 * name its filenames suggest:
 *
 * - **Android** — `app.config.ts` registers an explicit family named `WantedSans` with
 *   weights, so this string matches by construction.
 * - **iOS** — resolution goes through the name embedded in the file, which is
 *   `Wanted Sans` (with a space). Passing `WantedSans` there matches nothing and falls
 *   back to the system font silently.
 *
 * `Wanted Sans Medium` ships as a separate family and is handled per weight in
 * `../foundation/typography.ts`. See `assets/fonts/NOTICE.md`.
 */
