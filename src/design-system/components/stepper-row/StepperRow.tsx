...
function NumberIcon({ number, style }: NumberIconProps) {
  ...
  // The fill is injected here rather than baked into `styles.numberIcon`,
  // because StyleSheet.create runs at module scope where the theme is not
  // reachable. It stays ahead of `style` so callers can still override it.
  return (
    <View
      style={[styles.numberIcon, { backgroundColor: token.accent.fillColor }, style]}
    >
...
const styles = StyleSheet.create({
  ...
  numberIcon: {
    ...
    height: 28,
    borderRadius: 14,
    // backgroundColor is applied at render time from the theme accent.
    alignItems: 'center',
    justifyContent: 'center',
  },
  texts: {
